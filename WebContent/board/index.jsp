...
<header class="masthead d-flex">
	<div class="container text-center my-auto">
		<h1 class="mb-1 text-right">한 눈에 보자</h1>
...
<script type="text/javascript">
	...
	function foo(month){
		...
		// 처음 함수 실행 시 기존 적혀있던 내용들 삭제 출금 입금
		$('.withdrawal').remove();
		$('.deposit').remove();
		...
		for (var i = 0; i < data.length; i++) {
			...
			if (dataMonth == month) {
				...
				if (data[i].inout_type.includes('입금')) {
					...
					// 일자에 출력해준다. 입금위에찍고 출금밑으로찍기
					if ($('#date' + dataDate).children('#deposit').length > 0) { // 입금 표시가 있다면 내용만 바꿔라
						$('#date' + dataDate).children('#deposit').empty(); // 비워준다
						$('#date' + dataDate).children('#deposit').html( '+' + myDepositMap.get(dataDate)); // 받아와서 더해준다
